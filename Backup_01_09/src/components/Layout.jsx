...
export default function Layout({ children }) {
  ...
  // This can be used to apply different styles for active menu items
  const getMenuItemStyles = (path, isMinimized) => {
    const isActive = isActiveMenuItem(path);
    return {
      padding: isMinimized ? "0.9rem 0" : "0.9rem 1.5rem",
            ...
            {menuItems.map(item =>
              item.path ? (
                <Link
                  ...
                  style={getMenuItemStyles(item.path, minimized)}
                  tabIndex={0}
                  title={item.label}
