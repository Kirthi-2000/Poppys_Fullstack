...
const rawDataHeaders = [
  ...
  const fetchRawData = async () => {
    ...
    try {
      ...
      const res = await axios.get("https://oceanatlantic.pinesphere.co.in/api/poppys-machine-logs/raw/", { params });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axios.get("https://oceanatlantic.pinesphere.co.in/api/poppys-machine-logs/", { params });
...
const availableMachineIds = getAvailableMachineIds();
  ...
  const getTileDataFromFiltered = () => {
    ...
    const totalPieceCount = filtered.reduce((sum, row) => sum + (parseFloat(row.pieceCount) || 0), 0); // ✅ ADD THIS
