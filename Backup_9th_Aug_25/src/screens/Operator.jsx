...
// Modified table headers as per your request - ✅ ADD PIECE COUNT
const tableHeaders = [
  "S.No",
  "Date",
  "Operator ID",
...
export default function Operator({ tableOnly = false, from: propFrom = "", to: propTo = "", machineId, selectedOperatorId, lineId }) {
  ...
  useEffect(() => {
    const fetchOperatorOptions = async () => {
      try {
        const res = await axios.get("https://oceanatlantic.pinesphere.co.in/api/operator-report/");
  ...
  const fetchRawData = async () => {
    ...
    try {
      const params = {};
      // ✅ REMOVED: Don't send operator_id to API, we'll filter on frontend

      console.log("Operator raw data request params:", params);

      const res = await axios.get("https://oceanatlantic.pinesphere.co.in/api/operator-report/raw/", { params });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.get("https://oceanatlantic.pinesphere.co.in/api/operator-report/", { params });
      ...
      setError("Failed to fetch operator data"); // ✅ SET ERROR
      setData([]);
    }
    setLoading(false);
  };
  ...
  // Update filtering logic for summary table
  const applyFilters = (dataToFilter) => {
    ...
    // Multi-select Operator ID filter (fix here)
    if (selectedOperatorIds.length > 0) {
      filtered = filtered.filter(row =>
        row.operatorId && selectedOperatorIds.includes(row.operatorId.toString())
      );
    }
  ...
  // ✅ FIXED: Apply filters to raw data - Updated to use multi-select operator IDs
  const applyRawDataFilters = (dataToFilter) => {
    ...
    // ✅ FIXED: Apply multi-select operator ID filter (same as summary table)
    if (selectedOperatorIds.length > 0) {
      filtered = filtered.filter(row =>
        row.operatorId && selectedOperatorIds.includes(row.operatorId.toString())
      );
  ...
  const getTileDataFromFiltered = () => {
    if (filtered.length === 0) {
      return [
        ...
        {
          label: "Piece Count", // ✅ ADD PIECE COUNT TILE
          value: "0",
          bg: "tile-bg-purple",
    ...
    return (
      <div className="machine-root">
        <div style={{ 
          ...
          <h3>Error Loading Operator Report</h3>
          <p>{error}</p>
          <button 
            onClick={() => {
              setError(null);
      ...
      <div className="machine-table-card" style={{ marginBottom: "16px", padding: "16px" }}>
        <div className="machine-header-actions" style={{ display: "flex", justifyContent: "center", alignItems: "center", marginBottom: 16 }}>
          <div style={{ display: "flex", gap: 25, alignItems: "center" }}>
            ...
            {/* Multi-select Operator Dropdown */}
            <div className="operator-dropdown-wrapper">
              <button
                type="button"
                className="operator-select"
                onClick={() => setShowOperatorDropdown((v) => !v)}
              >
                {selectedOperatorIds.length === 0
                  ? "Select Operator ID(s)"
                  : selectedOperatorIds.length <= 2
                    ? selectedOperatorIds.join(", ")
                    : `${selectedOperatorIds.slice(0, 2).join(", ")} +${selectedOperatorIds.length - 2} more`}
                <span className="operator-select-arrow">▼</span>
              </button>
              {showOperatorDropdown && (
                <div className="operator-dropdown-list">
                  {/* Select All */}
                  <label className="operator-dropdown-item operator-dropdown-selectall">
                    <input
                      type="checkbox"
                      checked={availableOperatorIds.length > 0 && availableOperatorIds.every(id => selectedOperatorIds.includes(id.toString()))}
                      onChange={e => {
                        if (e.target.checked) {
                          setSelectedOperatorIds(availableOperatorIds.map(id => id.toString()));
                        } else {
                          setSelectedOperatorIds([]);
                        }
                      }}
                  ...
                  {availableOperatorIds.map(id => (
                    <label key={id} className="operator-dropdown-item">
                      <input
                        type="checkbox"
                        checked={selectedOperatorIds.includes(id.toString())}
                        onChange={() => {
                          setSelectedOperatorIds(prev =>
                            prev.includes(id.toString())
                              ? prev.filter(oid => oid !== id.toString())
                              : [...prev, id.toString()]
                          );
                        }}
                        className="machine-checkbox"
        ...
        {/* Filter Status */}
        {(from || to || selectedOperatorIds.length > 0) && (
          <div style={{
            padding: "8px 16px",
            marginBottom: "16px",
            backgroundColor: (showRawData ? filteredRawData.length > 0 : filtered.length > 0) ? "#e3f2fd" : "#ffebee", // ✅ FIXED: Check appropriate filtered data
            borderRadius: "4px",
            fontSize: "14px",
            color: (showRawData ? filteredRawData.length > 0 : filtered.length > 0) ? "#1976d2" : "#d32f2f" // ✅ FIXED: Check appropriate filtered data
          }}>
            ...
            {selectedOperatorIds.length > 0 && (
              <span style={{ marginLeft: "8px" }}>
                Operator(s): {selectedOperatorIds.join(", ")}
              </span>
            )}
            <span style={{ marginLeft: "8px" }}>
          ...
          <div className="machine-table-card">
            <div style={{
              ...
              <h3 style={{ color: '#e65100', marginBottom: '10px', fontSize: '24px' }}>
                Operator Raw Data Dashboard
              </h3>
              <p style={{ color: '#f57c00', fontSize: '16px', marginBottom: '30px', maxWidth: '400px' }}>
                Select your filters and click <strong>Generate</strong> to view detailed operator raw data and logs
              </p>
              <div style={{
                padding: '15px 25px',
                ...
                💡 Tip: Raw data shows individual operator log entries with detailed timestamps
              </div>
            </div>
          </div>
        ) : (
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td
                        ...
                        className="machine-table-nodata"
                      >
                        {from || to || selectedOperatorIds.length > 0 // ✅ FIXED: Check selectedOperatorIds instead of operatorId
          ...
          <div className="operator-table-card">
            <div style={{
              ...
              <p style={{ color: '#6c757d', fontSize: '16px', marginBottom: '30px', maxWidth: '400px' }}>
                Select your filters and click <strong>Generate</strong> to view operator data and analytics
              </p>
              <div style={{
                padding: '15px 25px',
                ...
                💡 Tip: Use filters to analyze specific operators or time periods
              </div>
            </div>
          </div>
        ) : (
                ...
                <table className="machine-table" style={{ tableLayout: "auto", width: "100%" }}>
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan={tableHeaders.length} className="machine-table-nodata">
                          {(from || to || operatorId) ? "No data found for the selected filters." : "No data found."}
                        </td>
                      </tr>
                    ) : (
                      paginated.map((row, idx) => (
