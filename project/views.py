...
# Elastic Search - Machine Report Module
class MachineIdSuggestView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Search Machine IDs that contain the query string
        machine_ids = (
            MachineLog.objects
            .filter(MACHINE_ID__icontains=query)
            .values_list('MACHINE_ID', flat=True)
            .distinct()
        )
        # Return as strings for frontend compatibility
        return Response({"machine_ids": [str(mid) for mid in machine_ids][:10]})

# Elastic Search - Operator Report Module
class OperatorIdSuggestView(APIView):
    permission_classes = [AllowAny]

    def get(self, request, *args, **kwargs):
        query = request.query_params.get('q', '').strip()
        if not query:
            return Response({"operator_ids": []})

        operator_ids = (
            MachineLog.objects
            .filter(OPERATOR_ID__icontains=query)
            .values_list('OPERATOR_ID', flat=True)
            .distinct()
        )
        return Response({"operator_ids": [str(oid) for oid in operator_ids][:10]})

# Elastic Search - Line Report Module
class LineIdSuggestView(APIView):
    permission_classes = [AllowAny]
    ...
    def get(self, request, *args, **kwargs):
        ...
        line_ids = (
            MachineLog.objects
            .filter(LINE_NUMB__icontains=query)
            .values_list('LINE_NUMB', flat=True)
            .distinct()
        )
        return Response({"line_ids": [str(lid) for lid in line_ids][:10]})
    

# Elastic Search - Consolidated Report Module (for dropdown suggestions)
class ConsolidatedMachineIdSuggestView(APIView):
    permission_classes = [AllowAny]
...
class ConsolidatedOperatorIdSuggestView(APIView):
    permission_classes = [AllowAny]
...
class ConsolidatedLineIdSuggestView(APIView):
    permission_classes = [AllowAny]
    ...
    def post(self, request, *args, **kwargs):
        ...
        if str_logid > 1000:
            ...
            if exists:
                ...
                return Response({"Code": 201, "Message": "Data not stored"}, status=201)
        ...
        if serializer.is_valid():
            ...
            return Response({"Code": 200, "Message": "Data stored successfully"}, status=200)
...
class MachineReport(APIView):    
    ...
    def get(self, request, *args, **kwargs):
        ...
        start_window = time(8, 45)
        end_window = time(18, 45)
        """ breaks = [
        ...
        for report_date in report_dates:
            ...
            for log in logs:
                ...
                # Add piece count to all modes (not just Mode 1)
                if piece_count >= 0:  # Include 0 values for accurate totals
        ...
        for idx, (report_key, data) in enumerate(report.items(), 1):
            ...
            # Calculate Idle Hours to ensure total is 10 hours
            work_sum = (
                data["Sewing Hours"] +
                data["No feeding Hours"] +
                data["Meeting Hours"] +
                data["Maintenance Hours"] +
                data.get("Rework Hours", 0) +
                data.get("Needle Break", 0)
            )
            
            if work_sum >= 10:
                idle_hours = 0
            else:
                idle_hours = 10 - work_sum
            summary[-1]["Idle Hours"] = hours_to_hhmm(idle_hours)
...
class LineReport(APIView):
        ...
        def get(self, request, *args, **kwargs):
            ...
            start_window = time(8, 45)   # Work starts at 08:45
            end_window = time(18, 45)    # Work ends at 07:45 PM
            """ breaks = [
...
class OperatorReport(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        start_window = time(8, 45)   # Work starts at 08:30
        end_window = time(18, 45)    # Work ends at 19:30
        """ breaks = [
            (time(10, 30), time(10, 40)),  # Morning break: 10:30-10:40
            (time(13, 20), time(14, 0)),   # Lunch break: 13:20-14:00
            (time(16, 20), time(16, 30)),  # Evening break: 16:20-16:30
